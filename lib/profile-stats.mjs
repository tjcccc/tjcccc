...
function buildRequestHeaders(username, githubToken) {
  const headers = {
...
async function githubJson(path, requestHeaders) {
...
async function githubJsonPages(path, requestHeaders) {
  const results = [];
  let page = 1;

  while (true) {
    const separator = path.includes("?") ? "&" : "?";
    const rows = await githubJson(`${path}${separator}per_page=100&page=${page}`, requestHeaders);
    results.push(...rows);
...
export function combinedCard(user, repos, languageTotals, displayName) {
  ...
  const title = `${displayName}'s GitHub stats`;
  const stats = [
...
export async function generateProfileStatsSvg(options = {}) {
  const username = options.username || process.env.PROFILE_USERNAME || "tjcccc";
  const displayName = options.displayName || process.env.PROFILE_DISPLAY_NAME || "seesaw game";
  const githubToken = options.githubToken || process.env.GITHUB_TOKEN || "";
  const requestHeaders = buildRequestHeaders(username, githubToken);

  const [user, repos] = await Promise.all([
    githubJson(`/${["users", username].join("/")}`, requestHeaders),
    githubJsonPages(`/${["users", username, "repos"].join("/")}?type=owner&sort=updated&direction=desc`, requestHeaders),
  ]);

  const sourceRepos = repos.filter((repo) => !repo.fork && !repo.archived && repo.languages_url);
  const languageResponses = await Promise.all(
    sourceRepos.map((repo) => githubJson(repo.languages_url.replace(API_ROOT, ""), requestHeaders)),
  );
  ...
  return combinedCard(user, repos, languageTotals, displayName);
}
